...
class Plugin {
	...
	/**
	 * AJAX action for dismissing a notice.
	 *
	 * Doubles as the nonce action, so the hook, the nonce and the check
	 * cannot drift apart.
	 *
	 * @since 1.0.0
	 */
	private const DISMISS_ACTION = 'wp_register_plugin_dismiss_notice';
	...
	public function __construct( array $config = [] ) {
		...
		$this->config = wp_parse_args( $config, [
			...
			'requirements_url' => '',
			'support_url'      => '',
			'error_message'    => __( 'This plugin is not fully active.', 'arraypress' ),
			'css_class_prefix' => 'arraypress',
			'plugin_links'     => [],
			'plugin_meta'      => [],
			'notices'          => [],
		] );
	...
	public function load_textdomain(): void {
		...
		load_plugin_textdomain( 'arraypress' );
	...
	public function error_plugin_action_links( array $links ): array {
		if ( ! empty( $this->config['requirements_url'] ) ) {
			$requirements_link = sprintf(
					...
					esc_attr__( 'View plugin requirements', 'arraypress' ),
					esc_html__( 'Requirements', 'arraypress' )
			);
		...
		if ( ! empty( $this->config['support_url'] ) ) {
			$support_link = sprintf(
					...
					esc_attr__( 'Get plugin support', 'arraypress' ),
					esc_html__( 'Support', 'arraypress' )
			);
	...
	public function dismiss_notice(): void {
		// A capability check answers "may this user dismiss notices", not
		// "did this user ask to". Without a nonce, any page an administrator
		// visits can make their browser post here, and the handler writes
		// user meta. Dismissing a notice is a small thing to lose, but a
		// state-changing endpoint with no CSRF protection is not.
	...
	private function should_show_notice( string $notice_id, array $notice, $current_screen ): bool {
		...
		if ( ! empty( $notice['pages'] ) && is_array( $notice['pages'] ) ) {
			...
			if ( ! in_array( $current_page, $allowed_pages, true ) && ! in_array( str_replace( '_', '-', $current_page ), $notice['pages'], true ) ) {
				return false;
			}
		}
		...
		<script type="text/javascript">
			jQuery(document).ready(function ($) {
				$(document).on('click', '.notice[data-notice-id] .notice-dismiss', function () {
					...
					if (noticeId) {
						$.post(ajaxurl, {
							action: <?php echo wp_json_encode( self::DISMISS_ACTION ); ?>,
							notice_id: noticeId,
							_ajax_nonce: <?php echo wp_json_encode( wp_create_nonce( self::DISMISS_ACTION ) ); ?>
						});
					}
				});
			});
